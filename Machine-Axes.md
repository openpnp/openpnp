...
Open Source controller firmwares often have 3D-printing heritage, where there were only `X` `Y` `Z` axes, plus a limited-capabilities axis `E` for the extruder. Later these firmwares were extended to support more axes, e.g. `A` `B` `C`, with full capabilities like reporting their positions. Make sure to have one of these modern firmwares loaded and the extra axes configured properly. See [[Motion Controller Firmwares]].
