...
There is a way to safely use all-positive Z coordinates. If they are truly all-positive, i.e. > Z0, even the very lowest coordinates you ever need to reach. If you define it that way, you can set the **Soft Limit Low** at Z=1 or similar (see [below](#kinematic-settings--axis-limits)). Any attempt to go to a Z coordinate you forgot to setup, i.e. to Z=0, will be safely caught, even before the machine starts to move there.
