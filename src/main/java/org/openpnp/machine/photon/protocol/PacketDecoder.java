...
public class PacketDecoder {
    ...
    private PacketDecoder(String message) {
        ...
        for (int i = 0; i < data.length; i++) {
            data[i] = PacketHelper.getByteAtPhoton(message, i);
        }

        int expectedChecksum = (PacketHelper.getByteAtPhoton(message, data.length + 1) << 8) +
                PacketHelper.getByteAtPhoton(message, data.length);
