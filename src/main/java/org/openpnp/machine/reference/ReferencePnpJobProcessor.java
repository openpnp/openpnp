...
public class ReferencePnpJobProcessor extends AbstractPnpJobProcessor {
    ...
    public synchronized void initialize(Job job) throws Exception {
        ...
        this.fireJobState(Configuration.get()
                                       .getMachine()
                                       .getSignalers(),
    ...
    protected class PreFlight implements Step {
        ...
        private void checkJobPlacement(JobPlacement jobPlacement) throws JobProcessorException {
            ...
            if (placement.getPart() == null) {
                throw new JobProcessorException(placement, String.format(
                        "Part not found for board %s, placement %s.", boardLocation.getBoard()
                                                                                   .getName(),
                        placement.getId()));
            }
            ...
            if (part.getPackage() == null) {
                throw new JobProcessorException(part,
                        String.format("No package set for part %s.", part.getId()));
            }

            // Verify that the part height is greater than zero. Catches a common configuration
            // error.
            if (placement.getPart()
                         .getHeight()
                         .getValue() <= 0D) {
                throw new JobProcessorException(placement.getPart(),
        ...
        private void validatePartNozzleTip(Head head, Part part) throws JobProcessorException {
            /**
             * 1. Make a list of NozzleTips that can be loaded into at least one Nozzle. 2. Filter
             * that list down to NozzleTips that can handle the part. 3. Return !list.isEmpty()
             */

            Set<NozzleTip> compatibleNozzleTips = head.getNozzles()
                                                      .stream()
                                                      .flatMap(nozzle -> {
                                                          return nozzle.getCompatibleNozzleTips()
                                                                       .stream();
                                                      })
                                                      .filter(nozzleTip -> {
            ...
            if (compatibleNozzleTips.isEmpty()) {
                throw new JobProcessorException(part, String.format(
                        "No compatible, loadable nozzle tip found for part %s.", part.getId()));
            }
        }
            ...
            for (Placement placement : boardLocation.getBoard()
                ...
                if (idlist.contains(placement.getId())) {
                    throw new JobProcessorException(boardLocation,
                            String.format(
                                    "This board contains at least one duplicate ID entry: %s ",
                                    placement.getId()));
                }
                else {
    ...
    protected class PanelFiducialCheck implements Step {
        public Step step() throws JobProcessorException {
            FiducialLocator locator = Configuration.get()
                                                   .getMachine()
                                                   .getFiducialLocator();

            if (job.isUsingPanel() && job.getPanels()
                                         .get(0)
                                         .isCheckFiducials()) {
                Panel p = job.getPanels()
                             .get(0);

                BoardLocation boardLocation = job.getBoardLocations()
                                                 .get(0);
    ...
    protected class ChangeNozzleTips extends PlannedPlacementStep {
        ...
        public Step stepImpl(PlannedPlacement plannedPlacement) throws JobProcessorException {
            ...
            fireTextStatus("Change nozzle tip on nozzle %s to %s.", nozzle.getName(),
                    nozzleTip.getName());
            try {
    ...
    protected class Pick extends PlannedPlacementStep {
        ...
        public Step stepImpl(PlannedPlacement plannedPlacement) throws JobProcessorException {
            ...
            /**
             * If either postPick or checkPartOn fails we discard and then cycle back to feed up to
             * pickRetryCount times. We include postPick because if the pick succeeded then we
             * assume we are carrying a part, so we want to make sure to discard it if there is a
             * problem.
             */
            try {
        ...
        private void pick(Nozzle nozzle, Feeder feeder, Placement placement, Part part)
        ...
        private void align(PlannedPlacement plannedPlacement, PartAlignment partAlignment)
            ...
            for (int i = 0; i < 3; i++) {
                ...
                try {
                    ...
                    Logger.debug("Align {} with {}, offsets {}", part, nozzle,
                            plannedPlacement.alignmentOffsets);
                    return;
                }
                catch (Exception e) {
                    lastException = e;
            ...
            try {
                if (!nozzle.isPartOn()) {
                    throw new JobProcessorException(nozzle,
                            "No part detected after alignment. Part may have been lost in transit.");
                }
            }
            catch (JobProcessorException e) {
                throw e;
    ...
    protected class Place extends PlannedPlacementStep {
        ...
        public Step stepImpl(PlannedPlacement plannedPlacement) throws JobProcessorException {
            ...
            boardLocation.setPlaced(jobPlacement.getPlacement()
                                                .getId(),
                    true);
        ...
        private void place(Nozzle nozzle, Part part, Placement placement,
                Location placementLocation) throws JobProcessorException {
        ...
        private void checkPartOn(Nozzle nozzle) throws JobProcessorException {
            ...
            try {
                if (!nozzle.isPartOn()) {
                    throw new JobProcessorException(nozzle,
                            "No part detected on nozzle before place.");
                }
            }
            catch (JobProcessorException e) {
                throw e;
        ...
        private void scriptBeforeAssembly(PlannedPlacement plannedPlacement,
                Location placementLocation) throws JobProcessorException {
            final Nozzle nozzle = plannedPlacement.nozzle;
            final JobPlacement jobPlacement = plannedPlacement.jobPlacement;
            final Placement placement = jobPlacement.getPlacement();
            ...
            try {
                ...
                Configuration.get()
                             .getScripting()
                             .on("Job.Placement.BeforeAssembly", params);
            }
            catch (Exception e) {
            }
        }
        ...
        private void scriptComplete(PlannedPlacement plannedPlacement, Location placementLocation)
            ...
            try {
                ...
                Configuration.get()
                             .getScripting()
                             .on("Job.Placement.Complete", params);
            }
            catch (Exception e) {
                throw new JobProcessorException(null, e);
            }
        ...
        private Location getPlacementLocation(PlannedPlacement plannedPlacement) {
            ...
            placementLocation = placementLocation.add(new Location(part.getHeight()
                                                                       .getUnits(),
                    0, 0, part.getHeight()
                              .getValue(),
                    0));
    ...
    protected class Finish implements Step {
        public Step step() throws JobProcessorException {
            ...
            List<JobPlacement> erroredPlacements = jobPlacements.stream()
                                                                .filter(jp -> {
            ...
            if (!erroredPlacements.isEmpty()) {
                ...
                fireTextStatus("Job finished without error, placed %s parts in %s sec. (%s CPH)",
                        totalPartsPlaced, df.format(dtSec),
                        df.format(totalPartsPlaced / (dtSec / 3600.0)));
            }

            Logger.info("Errored Placements:");
            for (JobPlacement jobPlacement : erroredPlacements) {
                Logger.info("{}: {}", jobPlacement, jobPlacement.getError()
                                                                .getMessage());
            }
    ...
    protected abstract class PlannedPlacementStep implements Step {
         ...
         * that needs to be done with that planned placement before returning. If there is an error
         * that must be handled by the user in real time the method may throw
         * PnpJobProcessorException. If there is an unrecoverable error with a placement the method
         * should setError() on the JobPlacement.
         * 
         * @param plannedPlacement The plannedPlacement to process, or null if there are no more to
         *        process. Null is a special case which means "Return the next step."
         * @return
         * @throws JobProcessorException
         */
        ...
        /**
         * Find the next uncompleted, non-errored PlannedPlacement and pass it to stepImpl. If
         * stepImpl completes without error the PlannedPlacement is marked complete and control is
         * returned to the caller.
         */
        public Step step() throws JobProcessorException {
            /**
             * Get the first planned placement from the list that is still in processing status and
    ...
    public static class SimplePnpJobPlanner implements PnpJobPlanner {
        /**
         * A simple two-pass planner which tries to fill each nozzle with a placement on each cycle
         * while minimizing nozzle tip changes.
         * 
         * The first pass tries to find a placement for each nozzle which will not require a nozzle
         * tip change.
         * 
         * The second pass allows nozzle tip changes while respecting any already used nozzle tips
         * for the cycle.
         */
        @Override
        public List<PlannedPlacement> plan(Head head, List<JobPlacement> jobPlacements) {
            ...
            List<NozzleTip> nozzleTips = new ArrayList<>(head.getMachine()
                                                             .getNozzleTips());
            ...
            for (Nozzle nozzle : new ArrayList<>(nozzles)) {
                PlannedPlacement plannedPlacement =
                        planWithoutNozzleTipChange(nozzle, jobPlacements);
                if (plannedPlacement != null) {
            ...
            for (Nozzle nozzle : new ArrayList<>(nozzles)) {
                PlannedPlacement plannedPlacement =
                        planWithNozzleTipChange(nozzle, jobPlacements, nozzleTips);
                if (plannedPlacement != null) {
                    plannedPlacements.add(plannedPlacement);
                    jobPlacements.remove(plannedPlacement.jobPlacement);
                    nozzles.remove(plannedPlacement.nozzle);
        ...
        protected PlannedPlacement planWithoutNozzleTipChange(Nozzle nozzle,
            ...
            for (JobPlacement jobPlacement : jobPlacements) {
                ...
                if (pkg.getCompatibleNozzleTips()
                       .contains(nozzleTip)) {
        ...
        protected PlannedPlacement planWithNozzleTipChange(Nozzle nozzle,
                List<JobPlacement> jobPlacements, List<NozzleTip> nozzleTips) {
            for (JobPlacement jobPlacement : jobPlacements) {
                Placement placement = jobPlacement.getPlacement();
                Part part = placement.getPart();
                org.openpnp.model.Package pkg = part.getPackage();
                ...
                List<NozzleTip> goodNozzleTips = nozzleTips.stream()
                                                           .filter(nozzleTip -> {
                                                               return pkg.getCompatibleNozzleTips()
                                                                         .contains(nozzleTip);
                                                           })
                                                           .filter(nozzleTip -> {
                                                               return nozzle.getCompatibleNozzleTips()
