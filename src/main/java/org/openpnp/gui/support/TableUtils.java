    ...
    private static void restoreColumnWidths(JTable table, Preferences prefs, String prefKey) {
        SwingUtilities.invokeLater(() -> {
            ...
            for (int iCol=0; iCol<nCols; iCol++) {
                prefWidths[iCol] = prefs.getInt(prefKey + iCol, -1);
                if (prefWidths[iCol] < 0) {
                    prefWidths[iCol] = table.getColumnModel().getColumn(iCol).getWidth();
                    prefs.putInt(prefKey + iCol, prefWidths[iCol]);
                }
                if (widthTypes[iCol] == ColumnWidthSaveable.FIXED) {
                    totalFixed += prefWidths[iCol];
                }
                if (widthTypes[iCol] == ColumnWidthSaveable.PROPORTIONAL) {
            ...
            for (int iCol=0; iCol<nCols; iCol++) {
                if (widthTypes[iCol] == ColumnWidthSaveable.PROPORTIONAL || totalFixed == 0) {
            ...
            while (newTotalWidth < totalWidth) {
                if (widthTypes[i] == ColumnWidthSaveable.PROPORTIONAL || totalFixed == 0) {
                    newWidths[i]++;
                    newTotalWidth++;
                }
                i = (i+1) % nCols;
