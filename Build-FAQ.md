...
1. All Z heights that the pickup nozzle needs to pick from or place on, must be exactly the same. Specifically, this means that the top surface of all components in feeders must be equal, and the top of the PCB has to be very close also. The reason for this requirement is that OpenPnP's computer vision system needs to be able to translate from 1 pixel in the cameras image to a physical distance in the machine, and that distance varies with how far the object being measured is from the camera (close objects look bigger). It may seem like a small difference, but it's critical to get right.
...
1. The pickup nozzle at the end of the Z-axis ultimately has to be the most accurate and precise part of the machine as it's the 'business end'. It's also the part at the end of the mechanical chain (Y-axis -> X-axis -> head -> Z-axis -> pickup nozzle), so will be impacted by issues further up the chain. This is called the load path.
...
The current recommendation for a build guideline is [Stephen Hawes's Index](https://github.com/sphawes/index/). The design and build of every aspect has been extensively documented in a great, high-quality YouTube series.

For a more pick-and-choose build the recommendation is:
...
belts vs leadscrews and different types of each  
