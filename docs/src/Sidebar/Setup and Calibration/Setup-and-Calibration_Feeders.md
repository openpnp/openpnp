...
* **ReferenceDragFeeder**: A "drag" feeder which allows the use of an Actuator, typically a solenoid with a pin, to advance the tape by dragging it. Basic vision for part center detection is also supported. Note, the [[ReferencePushPullFeeder]] (above) can also cover drag feeding and has more features.

* **ReferenceLeverFeeder**: A feeder where a lever is pushed by an Actuator, typically the head, to advance the tape.  All part pitches are supported with multiple lever presses as required. Note, the [[ReferencePushPullFeeder]] (above) can also cover lever feeding and has more features.
