...
* The following animation quickly shows you how to fix a threshold using the mouse to probe image pixels. Probe the pixels that should be excluded and probe the pixels that should be included, then set the threshold to a value in between (use the context menu to display the gif separately and from the start):
