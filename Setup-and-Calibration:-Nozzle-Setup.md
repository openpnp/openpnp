...
**Maximum Pick Tolerance** sets the maximum allowed distance of the detected part, from its expected location, i.e., it represents the maximum expected offsets when picking a part from a feeder, including corner offsets resulting from rotational offsets. If the bottom vision detected offsets are larger, an error will be thrown. This tolerance also controls some stages in the pipeline, by limiting the maximum search distance and lessening the computational cost. Furthermore, this value is very important in [[Vision Compositing]] a.k.a. multi-shot bottom vision, where the tolerance applies to maximum corner offsets. A large tolerance can thwart the isolation and detection of corners and angles, the multi-shot will fail.
