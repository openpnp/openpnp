...
* **Once** probes for a Z offset only once. The offset is stored in the configuration (`machine.xml`). CAUTION: Z offsets from this first _one_ probeîng will subsequently be applied quietly, for ever. You might forget it is applied, and when you later modify the feeder/machine geometry etc. it might exhibit seemingly strange behavior. Look for applied offsets in the log at TRACE level (search for "applies feeder" or "applies part"). 
* **AfterHoming** probes for fresh Z offsets per feeder/per part once after homing the machine.  
